...
test('Element should serialize namespace', t => {
  ...
  t.is((svg.serialize() as TransferrableNode)[TransferrableKeys.namespaceURI], SVG_NAMESPACE);
});
...
test('Element should serialize child node as well', t => {
  ...
  const serializedDiv = div.serialize() as TransferrableNode;
  t.is(serializedDiv[TransferrableKeys.childNodes].length, 1);
  t.deepEqual(serializedDiv[TransferrableKeys.childNodes][0], childDiv.serialize());
});
