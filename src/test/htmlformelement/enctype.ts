...
test('enctype should be settable to a string value', t => {
  const { element } = t.context as { element: HTMLFormElement };

  element.enctype = 'multipart/form-data';
  t.is(element.enctype, 'multipart/form-data');
});

test('enctype property change should be reflected in attribute', t => {
  const { element } = t.context as { element: HTMLFormElement };

  element.enctype = 'multipart/form-data';
  t.is(element.getAttribute('enctype'), 'multipart/form-data');
});

test('enctype attribute change should be reflected in property', t => {
  const { element } = t.context as { element: HTMLFormElement };
