...
test('single direct child with one classname', t => {
  ...
  t.deepEqual(node.getElementsByClassName('foo')[0], child);
});
...
test('multiple direct children with two classnames', t => {
  ...
  t.deepEqual(node.getElementsByClassName('foo bar')[0], child);
  t.deepEqual(node.getElementsByClassName('foo')[0], child);
  t.deepEqual(node.getElementsByClassName('bar')[0], child);
});
