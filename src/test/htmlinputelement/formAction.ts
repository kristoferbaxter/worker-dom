...
test('formformAction should be empty by default', t => {
  const { element } = t.context as { element: HTMLInputElement };

  t.is(element.formAction, '');
});

test('formAction should be settable to a string value', t => {
  const { element } = t.context as { element: HTMLInputElement };

  element.formAction = '/publish';
  t.is(element.formAction, '/publish');
});
...
test('formAction property change should be reflected in attribute', t => {
  ...
  element.formAction = '/publish';
  t.is(element.getAttribute('formaction'), '/publish');
});

test('formAction attribute change should be reflected in property', t => {
  const { element } = t.context as { element: HTMLInputElement };

  element.setAttribute('formaction', '/publish');
  t.is(element.formAction, '/publish');
});
