...
test('formTarget should be settable to a string value', t => {
  const { element } = t.context as { element: HTMLInputElement };

  element.formTarget = 'some-target';
  t.is(element.formTarget, 'some-target');
});
...
test('formTarget property change should be reflected in attribute', t => {
  ...
  element.formTarget = 'some-target';
  t.is(element.getAttribute('formtarget'), 'some-target');
});

test('formTarget attribute change should be reflected in property', t => {
  const { element } = t.context as { element: HTMLInputElement };

  element.setAttribute('formtarget', 'some-target');
  t.is(element.formTarget, 'some-target');
});
