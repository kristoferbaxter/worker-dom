...
test('autocapitalize should be empty by default', t => {
  const { element } = t.context as { element: HTMLInputElement };

  t.is(element.autocapitalize, '');
});

test('autocapitalize should be settable to a string value', t => {
  const { element } = t.context as { element: HTMLInputElement };

  element.autocapitalize = 'chararacters';
  t.is(element.autocapitalize, 'chararacters');
});
...
test('autocapitalize property change should be reflected in attribute', t => {
  ...
  element.autocapitalize = 'chararacters';
  t.is(element.getAttribute('autocapitalize'), 'chararacters');
});

test('autocapitalize attribute change should be reflected in property', t => {
  const { element } = t.context as { element: HTMLInputElement };

  element.setAttribute('autocapitalize', 'chararacters');
  t.is(element.autocapitalize, 'chararacters');
});
