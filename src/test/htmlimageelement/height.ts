...
test('height should be zero by default', t => {
  const { element } = t.context as { element: HTMLImageElement };

  t.is(element.height, 0);
});

test('height should be settable to a number value', t => {
  const { element } = t.context as { element: HTMLImageElement };

  element.height = 100;
  t.is(element.height, 100);
});
...
test('height property change should be reflected in attribute', t => {
  ...
  element.height = 100;
  t.is(element.getAttribute('height'), '100');
});
